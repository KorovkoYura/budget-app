...
class App extends Component {
	...
	onToday = () => {
	    ...
	    const currentTrassacrions = currentMonthTransactions.reduce(
	    	(acc, transaction) =>
	    ...
	    const daysLeft = moment().daysInMonth() - date.date();

	    /*console.log({dailyMoney, expanseBeforeToday, incomeBeforeToday});*/

	    const onToday = parseInt((currentTrassacrions + expanseBeforeToday) / daysLeft, 10);
		  ...
		  <section>
		    ...
		    <main>
		    	...
		    	<Table>
		    		<tbody>
		    			{
		    				transactions.filter(({date: transactionDate}) => moment(transactionDate, 'DD.MM.YYYY')
		    				.isSame(date, 'month'))
